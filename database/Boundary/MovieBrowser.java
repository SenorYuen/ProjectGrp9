...
//    }
    ...
    private void reserveSeat(JFrame mainWindow) {
        ...
        // Check if user is logged in (if personId is 0 or not available, assume the user is a regular user)
        String userCardNumber = null;
        if (personId == 0) {
            // If not logged in, prompt the user to enter their card number
            userCardNumber = JOptionPane.showInputDialog("Please enter your card number:");
            if (userCardNumber == null || userCardNumber.trim().isEmpty()) {
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            // Pass the userCardNumber for regular users, or null for logged-in users
            String status = ticketService.reserveTicket(conn, personId == 0 ? null : personId, showtimeId, seatRow, seatColumn, userCardNumber);
